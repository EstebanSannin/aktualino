...
********************************************************************/
/*
 * AKTUALINO OVERRIDE COPY.
 * Derived from the vendored Berry v1.1.0 default (berry/default/berry_conf.h).
 * This is the config used by BOTH the coc codegen and the C compiler (coc is a
 * text macro-scanner and cannot follow #include_next, so config must be a single
 * file). berry/default/berry_conf.h is kept pristine but is NOT on the include
 * path — this file shadows it. When bumping Berry, reconcile this file against
 * the new upstream default (see berry/VENDORING.md).
 *
 * Aktualino changes vs upstream are tagged  // AKT:  below.
 */
#ifndef BERRY_CONF_H
#define BERRY_CONF_H
 ...
 * Default: 0
 **/
#define BE_USE_FILE_SYSTEM              0  // AKT: scripts never touch flash directly (delivery is Uptane, persistence is the host store.* API); also drops the FatFs dependency on ESP-IDF
 ...
 * Default: 1
 **/
#define BE_USE_SHARED_LIB               0  // AKT: no dynamic library loading on an MCU
...
#define BE_USE_OS_MODULE                0  // AKT: depends on the file system, and os.* (system/exec/paths) has no place in a sandboxed script
